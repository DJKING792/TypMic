...
<p align="center">手机浏览器扫码即连，语音走纯局域网、<b>音频不出内网</b>。云端 MiMo + 本地 Whisper / SenseVoice <b>多引擎识别</b>，配合术语表纠错与可选 AI 润色，让口述直接变成能用的文本。Windows / macOS / Linux 通用。</p>
...
![使用次数](https://img.shields.io/endpoint?url=https://gist.githubusercontent.com/DJKING792/d0ef5bb70b36d07cf5fc779adc2ecc1c/raw/stats_count.json)
![已输入字数](https://img.shields.io/endpoint?url=https://gist.githubusercontent.com/DJKING792/d0ef5bb70b36d07cf5fc779adc2ecc1c/raw/stats_chars.json)
...
| 识别能力  | 小米 MiMo 云端（中文 / 方言 / 中英混说）+ 本地 faster-whisper / SenseVoice 离线双模式 | 多为单一引擎 / 单方案                 |
...
**云端模式**需要联网（调用 MiMo 的 ASR 接口），但你的**音频只会经自己的局域网传到本机电脑**，不经过任何第三方中转。若要做到**完全不联网**，请开启**离线模式**（`TYPOMIC_ASR=whisper` 或 `=sensevoice`）：识别全程在本机运行，彻底离线可用。
