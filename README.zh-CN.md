...
- [快速开始](#快速开始)
  ...
  - [Windows](#windows)
  - [macOS](#macos)
  - [手机连接](CONNECT_PHONE.zh-CN.md)

## 与同类工具对比

| 维度    | PhoneMic（本项目）                                       | 多数同类方案                       |
| ----- | --------------------------------------------------- | ---------------------------- |
...
3. 双击 `start.bat`
4. 按手机系统完成连接（见 [手机连接详解](CONNECT_PHONE.zh-CN.md)）
...
启动后屏幕显示「手机访问地址」和二维码，按 [手机连接详解](CONNECT_PHONE.zh-CN.md) 操作。
...
**云端模式**需要联网（调用 MiMo 的 ASR 接口），但你的**音频只会经自己的局域网传到本机电脑**，不经过任何第三方中转。若要做到**完全不联网**，请开启**离线模式**（`PHONEMIC_ASR=local`）：识别全程在本机运行，彻底离线可用。
...
云端模式下，音频只会离开局域网去访问小米 ASR 接口做转写，PhoneMic 不存储任何内容；离线模式下音频则完全不出本机。
